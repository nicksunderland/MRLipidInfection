...
## Configuration file
The column names in any given GWAS may not be consistent with the `TwoSampleMR` package naming
...
we need to provide a `config.yml` file to the `GWAS` object constructor specifying the mapping of
column names. A copy of the file structure can be opened in `R` with the `get_default_gwas_config()`
- a copy of this should be saved and adjusted as needed.

```{r config_file, eval = TRUE}
# File path to the config file; set open_file=TRUE to also opens the file in RStudio
fp <- get_default_gwas_config(open_file=FALSE)

# View the structure of the config file
...
for the lipid components: HDL-C, LDL-C, nonHDL-C, TC and TG. As we will be using outcome data from
the UK BioBank we will use the data excluding the UK BioBank to avoid overlap of the exposure and
outcome populations.
...
**Warning** The GWAS data files are large and will take some time to download. Creating further
`GWAS` objects with the same `file_path` will result in much quicker loading as the class first
checks whether the file has already been downloed and extracted.

There are more than 47 million SNPs in the datasets. We need to filter the GWAS data SNPs associated
with circulating lipid levels at a statistical significance of `p<5e-8`. This threshold is passed to
the `GWAS` object at instantiation. Internally, the `GWAS` caches the filters SNPs to a .csv file
within the package structure. This is loaded instead if the command is re-run (behaviour that can be
overridden by passing `overwrite=TRUE` to the `GWAS` constructor).

```{r download_data, eval = FALSE}
# Create a list of GWAS objects
...
(number of bases) to the genes of interest (HMG-CoA Reductase, NPC1L1, PCSK9, CETP).

Data regarding the positions of the genes is available
[online](https://www.genecards.org/cgi-bin/carddisp.pl?gene=HMGCR).
...
First I create a list of objects of class `GenomeRegion` - this is a simple custom S4 class
containing gene information. In addition, we will look at all of the SNPs across the whole genome
that associate with lipid profile components (set `genome_wide=TRUE`).

```{r genome_regions, eval = FALSE}
# The HMG-CoA-R, NPC1L1, and PCSK9 genes; set the 'cis' base tolerance here.
genome_regions_lst = list(
  GenomeRegion(name="HMGCR",  genome_wide=FALSE, chromosome=5,  start=74632993, end=74657941, cis_tol=1e5, clump_r2=0.1),
  GenomeRegion(name="NPC1L1", genome_wide=FALSE, chromosome=7,  start=44552134, end=44580929, cis_tol=1e5, clump_r2=0.1),
  GenomeRegion(name="PCSK9",  genome_wide=FALSE, chromosome=1,  start=55505221, end=55530525, cis_tol=1e5, clump_r2=0.1),
  GenomeRegion(name="CETP",   genome_wide=FALSE, chromosome=16, start=56995862, end=57017757, cis_tol=1e5, clump_r2=0.1),
...
# Create the permutations of lipid components and genome regions
lipids_gwas_lst <- purrr::map2(
...
# Run clumping
lipids_gwas_lst <- purrr::map(lipids_gwas_lst, clump_snps)
```
...
outcome_gwas_id <- c("ieu-b-5086")
# "ieu-b-5088", "ieu-b-4982", 
...
outcome_data <- purrr::map(.x = lipids_gwas_lst,
...
zero_snps <- purrr::map_lgl(lipids_gwas_lst, ~ sum(.x@use_flag)==0)

# Harmonise the data for the LDL SNPs across the whole genome
harm_dat <- purrr::map2(.x = lipids_gwas_lst[!zero_snps],
...
png(file="vignettes/figures/mr_heatmap.png", width=7, height=7, units="in", res=100)
p
dev.off()
```
...
scatter_plots <- purrr::map2(.x = res_dat, 
                             .y = harm_dat,
                             .f = ~TwoSampleMR::mr_scatter_plot(.x, .y)) %>% unlist(recursive = FALSE)

gene = "CETP"
gwas_id = "ieu-b-5086"
plots_of_interest <- scatter_plots[grepl(paste0(gene,".",gwas_id), names(scatter_plots))]
plots_of_interest
